...
def get_RH(data,temp,fold_save='',name=None):
    RH_list=[]
    data_list=[]
...
def main(key_words_list,key_common,delta_R=True,base_R=0,fold='./',if_save=False,fold_save=None,bridge=1,temp=None):
    ...
    for key_words in key_words_list:
        ...
        dat=np.loadtxt(fold+'/'+name,unpack=True,usecols=(3,4,bridge_number*2+4),skiprows=33,delimiter=',')

        dat[2]=-dat[2]
        dat_list,temp=sep_data(dat,temp=temp)
        get_RH(dat_list,temp,fold_save=fold_save,name=f)
        dat=dat[1:]
...
if __name__=='__main__':
    key_words_list=['300_175']
    key_words_common=['0406','2p48']
    temp=[175,200,300][:]
    fold=r'D:\OneDrive - tju.edu.cn\科研\CrPt\data\ppms\2023\202304'
